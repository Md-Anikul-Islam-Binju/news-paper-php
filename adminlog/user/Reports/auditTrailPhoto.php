...
<head>
<title>Audit Trail - List (Photo)</title>
...
<div class="DContent">
	...
	<tr><th colspan="8">Audit Trail - List (Photo)</th></tr>
	<tr><td colspan="8" class="TdSearch">
	<form name="frmContentID" action="auditTrailPhoto.php" method="post">
	...
	$qContent="SELECT * FROM com_audittrail_photo ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";

	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){
			$iContentID=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_photo WHERE QueryDetails LIKE '%".$iContentID."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
		if($_REQUEST["cboSearchType"]==2){
			$sSearchText=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_photo WHERE QueryDetails LIKE '%".$sSearchText."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
	}
	//echo $qContent."<br>";
	...
	<tr class="TrUpdateListSelect">
		...
		<td align="left" valign="top"><?php if($rsContent["ActionType"]==1){echo "Insert";}elseif($rsContent["ActionType"]==2){echo "<font color=blue>Update</font>";}elseif($rsContent["ActionType"]==3){echo "<font color=red>Delete</font>";}elseif($rsContent["ActionType"]==4){echo "<font color=sky>Activated</font>";} ?></td>
	...
	<?php $qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_photo";
	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_photo WHERE QueryDetails LIKE '%".$iContentID."%' ";}
		if($_REQUEST["cboSearchType"]==2){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_photo WHERE QueryDetails LIKE '%".$sSearchText."%' ";}
	}
