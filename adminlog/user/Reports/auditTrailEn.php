...
<head>
<title>Audit Trail - List (Content - English)</title>
...
<div class="DContent">
	...
	<tr><th colspan="8">Audit Trail - List (Content - English)</th></tr>
	<tr><td colspan="8" class="TdSearch">
	<form name="frmContentID" action="auditTrailEn.php" method="post">
	...
	$qContent="SELECT * FROM com_audittrail_encontent ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
	
	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){
			$iContentID=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_encontent WHERE QueryDetails LIKE '%".$iContentID."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
		if($_REQUEST["cboSearchType"]==2){
			$sSearchText=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_encontent WHERE QueryDetails LIKE '%".$sSearchText."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
	}
	...
	<?php $qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_encontent";
	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_encontent WHERE QueryDetails LIKE '%".$iContentID."%' ";}
		if($_REQUEST["cboSearchType"]==2){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_encontent WHERE QueryDetails LIKE '%".$sSearchText."%' ";}
	}
	$result=mysqli_query($connEMMAudit, $qCoutner) or die("Error, query failed");
	$row=mysqli_fetch_assoc($result);$numrows=$row["numrows"];
