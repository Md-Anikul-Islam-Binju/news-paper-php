...
<head>
<title>Audit Trail - List (Common - English)</title>
...
<div class="DContent">
	...
	<tr><th colspan="8">Audit Trail - List (Common - English)</th></tr>
	<tr><td colspan="8" class="TdSearch">
	<form name="frmContentID" action="auditTrailCommonEn.php" method="post">
	...
	$qContent="SELECT * FROM com_audittrail_gen_en ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";

	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){
			$iContentID=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_gen_en WHERE QueryDetails LIKE '%".$iContentID."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
		if($_REQUEST["cboSearchType"]==2){
			$sSearchText=$_REQUEST["txtSearch"];
			$qContent="SELECT * FROM com_audittrail_gen_en WHERE QueryDetails LIKE '%".$sSearchText."%' ORDER BY AuditTrailID DESC LIMIT $offset, $rowsPerPage";
		}
	}
	//echo $qContent."<br>";
	...
	<?php $qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_gen_en";
	if(isset($_REQUEST["btnSubmit"])){
		if($_REQUEST["cboSearchType"]==1){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_gen_en WHERE QueryDetails LIKE '%".$iContentID."%' ";}
		if($_REQUEST["cboSearchType"]==2){$qCoutner="SELECT COUNT(AuditTrailID) AS numrows FROM com_audittrail_gen_en WHERE QueryDetails LIKE '%".$sSearchText."%' ";}
	}
