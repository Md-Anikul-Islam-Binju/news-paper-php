...
<head>
<title>Special Category - Update List (Bangla) Deleted </title>
...
<div class="DContent">
	...
	<tr><th colspan="8">Special Category - Update List (Bangla) Deleted </th></tr>
	<tr><td colspan="8" class="TdUpdateCat">
		<a href="speContentUpdateListD.php?specatid=0&page=1" class="AEn">ALL</a>
		...
		while($rsSpeCategory=mysqli_fetch_assoc($resultSpeCategory)){
			echo "<a href='speContentUpdateListD.php?specatid=".$rsSpeCategory["SpecialCategoryID"]."&page=1'>".$rsSpeCategory["SpecialCategoryName"]."</a>";
		}mysqli_free_result($resultSpeCategory); ?>
		<a href="speContentUpdateList.php" class="AEn ADel">Deleted Records</a>
	</td></tr>
	<tr><td colspan="8" class="TdSearch">
	<form name="frmContentID" action="speContentUpdateListD.php" method="post">
	...
	$iSpeCategoryID=0;$iSearchType=0;$sSearch="";
	if(isset($_REQUEST["page"])){$pageNum=$_REQUEST["page"];}
	if(isset($_REQUEST["specatid"])){$iSpeCategoryID=$_REQUEST["specatid"];}else{$iSpeCategoryID=0;}
	...
	<tr class="TrUpdateListSelect">
		...
		<div class="DCategory">
		Special Cat: <a href="speContentUpdateList.php?specatid=<?php echo $rsContent["SpecialCategoryID"]; ?>&page=1"><?php echo $rsContent["SpecialCategoryName"]; ?></a>
		...
		<?php if($rsContent["TopHome"]==1){
			...
			echo "<span class='SpnTop'>Top 2</span>";
		}else{
		...
		elseif($rsContent["SubCategoryIDPos"]==3){echo "Top 2";}
		else{echo "List";} ?>
		</div><br>
		<div class="DSubSpeCategory">
	...
	if($iSpeCategoryID>0){$qCoutner="SELECT COUNT(ContentID) AS numrows FROM bn_content WHERE SpecialCategoryID=".$iSpeCategoryID." AND Deletable=2";
	...
	for($page=1;$page<=$maxPage;$page++){if($page==$pageNum){$nav.=" $page ";}else{$nav.=" <a href=\"$self?specatid=$iSpeCategoryID&page=$page&SearchTxt=$sSearch\">$page</a> ";}}
	if($pageNum>1){
		$page=$pageNum-1;
		$prev=" <a href=\"$self?specatid=$iSpeCategoryID&page=$page&SearchTxt=$sSearch\">[Prev]</a> ";
		$first=" <a href=\"$self?specatid=$iSpeCategoryID&page=1&SearchTxt=$sSearch\">[First Page]</a> ";
	}else{$prev="&nbsp;";$first="&nbsp;";}
	if($pageNum<$maxPage){
		$page=$pageNum+1;
		$next=" <a href=\"$self?specatid=$iSpeCategoryID&page=$page&SearchTxt=$sSearch\">[Next]</a> ";
		$last=" <a href=\"$self?specatid=$iSpeCategoryID&page=$maxPage&SearchTxt=$sSearch\">[Last Page]</a> ";
