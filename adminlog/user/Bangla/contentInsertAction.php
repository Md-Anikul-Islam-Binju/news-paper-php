...
if(isset($_POST["btnSubmit"])){
	...
	$sSubHeading="";$sHeading="";$sWriter="";$sInitial=$sInitialBN;$sSource="";$sContentBrief="";$sContentDetails="";
	$sImageSMPath="";$sImageSMPathNew="";$sImageSMPathCaption="";$sImageBGPath="";$sImageBGPathNew="";$sImageBGPathCaption="";
	$sTagName="";$sSoundPath="";$sVideoPath="";$sURLAlies="";
	...
	$qInsert="INSERT INTO bn_content(CategoryID, TopHome, TopInner,
		...
		VALUES(".$iCategory.", ".$iTopHome.", ".$iTopInner.",
			...
			'".$sWriter."', '".$sInitial."', '".$sSource."', '".$sContentBrief."', '".$sContentDetails."',
			'".$sImageSMPathNew."', '".$sImageSMPathCaption."', '".$sImageBGPathNew."', '".$sImageBGPathCaption."',
			'".$sTagName."', '".$sVideoPath."', ".$iShowContent.", ".$iShowInScroll.", ".$iPreviousContentID.", '".$sURLAlies."', '".$dtDateTime."')";
	...
	if($resultInsert){
		...
		VALUES('".$_SESSION["sessUserName"]."', 1, ".$iThisContentID.", 'bn_content', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qInsert)."', '".$dtDateTime."')";
		...
		//Fix TopHome
		if($iTopHome==2){
			$qTopHome="UPDATE bn_content SET TopHome=3 WHERE TopHome=2 AND CategoryID=".$iCategory." AND ContentID!=".$iThisContentID;
		...
		if($iTopInner>1){
			if($iTopInner==2){
				$qTopInner="UPDATE bn_content SET TopInner=3 WHERE CategoryID=".$iCategory." AND TopInner=2 AND ContentID!=".$iThisContentID;
				//$qTopInner="UPDATE bn_content SET TopInner=3 WHERE ContentID!=".$iThisContentID;
			...
			if($iTopInner==3){
				//$qTopInner="UPDATE bn_content SET TopInner=1 WHERE CategoryID=".$iCategory." AND TopInner=2 AND ContentID!=".$iThisContentID;
				$qTopInner="UPDATE bn_content SET TopInner=1 WHERE ContentID IN
	(SELECT ContentID FROM
		(SELECT ContentID, CategoryID, TopHome, TopInner FROM bn_content WHERE CategoryID=".$iCategory." AND TopInner=2 AND ContentID!=".$iThisContentID." ORDER BY ContentID DESC LIMIT 5, 10)
	AS t)";
				//echo "qTopInner: ".$qTopInner."<br>";
				mysqli_query($connEMM, $qTopInner) or die("Update TopInner: ".mysqli_errno($connEMM).": ".mysqli_error($connEMM));
			}
		...
		if($iSubCategoryIDPos==2){
			$qSubCategoryIDPos="UPDATE bn_content SET SubCategoryIDPos=3 WHERE SubCategoryID=".$iSubCategoryID." AND SubCategoryIDPos=2 AND ContentID!=".$iThisContentID;
		...
		if($sImageSMPath!=""){
			if(move_uploaded_file($_FILES["txtImageSMPath"]["tmp_name"], $UploadImageAllSM.$_FILES["txtImageSMPath"]["name"])){
				...
				while($fileRen=readdir($dir)){
					if($fileRen==$sImageSMPath){
						...
						if($iFlag==1){
							...
							$qUpdate="UPDATE bn_content SET ImageSMPath='".$sNewNameSMDir."' WHERE ContentID=".$iThisContentID;
				...
				//echo "sNewNameThumb: ".$sNewNameThumb."<br>";
				$qUpdate="UPDATE bn_content SET ImageThumbPath='".$sNewNameThumb."' WHERE ContentID=".$iThisContentID;
		...
		if($sImageBGPath!=""){
				...
				while($fileRen=readdir($dir)){
					if($fileRen==$sImageBGPath){
						...
						if($iFlag==1){
							...
							$qUpdate="UPDATE bn_content SET ImageBGPath='".$sNewNameBGDir."' WHERE ContentID=".$iThisContentID;
			...
			if($sImageSMPath==""){
				...
				$qUpdate="UPDATE bn_content SET ImageSMPath='".$sNewNameBG."' WHERE ContentID=".$iThisContentID;
		...
		echo $sMsgInsert;
		$_SESSION["sessRedirectPageBN"]="contentInsert.php";//Collecting where to redirect
