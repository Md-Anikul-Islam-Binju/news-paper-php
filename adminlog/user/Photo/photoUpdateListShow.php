...
<head>
<title>Create Gallery - Update List (Show)</title>
...
<div class="DContent">
	...
	<tr><th colspan="4">Create Gallery - Update List (Show)</th></tr>
	<tr><td colspan="4" class="TdUpdateCat">
		<?php $qSQL="SELECT AlbumID, AlbumName FROM p_album WHERE Deletable=1 ORDER BY AlbumID DESC LIMIT 20";
			...
			while($rsAlbum=mysqli_fetch_assoc($resultAlbum)){
				echo "<a href='photoUpdateList.php?albumid=".$rsAlbum["AlbumID"]."'><b>". $rsAlbum["AlbumName"]."</b></a> &nbsp;&nbsp;&nbsp;&nbsp; ";
	...
	if($iAlbumID>0){
		$qSQL="SELECT p_photo.AlbumID, p_album.AlbumName, p_album.AlbumNameBN, p_photo.PhotoID, p_photo.CaptionBN, p_photo.ImagePath, p_photo.ImageType, p_photo.ShowAtHome, p_photo.DateTimeInserted, p_photo.DateTimeUpdated, p_photo.ContentID FROM p_album INNER JOIN p_photo ON p_photo.AlbumID=p_album.AlbumID WHERE p_photo.Deletable=1 AND p_photo.ShowAtHome=1 AND p_photo.AlbumID=".$iAlbumID." ORDER BY p_photo.PhotoID DESC LIMIT $offset, $rowsPerPage";
	}else{
		$qSQL="SELECT p_photo.AlbumID, p_album.AlbumName, p_album.AlbumNameBN, p_photo.PhotoID, p_photo.CaptionBN, p_photo.ImagePath, p_photo.ImageType, p_photo.ShowAtHome, p_photo.DateTimeInserted, p_photo.DateTimeUpdated, p_photo.ContentID FROM p_album INNER JOIN p_photo ON p_photo.AlbumID=p_album.AlbumID WHERE p_photo.Deletable=1 AND p_photo.ShowAtHome=1 ORDER BY p_photo.PhotoID DESC LIMIT $offset, $rowsPerPage";
	}
	if(isset($_REQUEST["txtPhotoID"])){
		$iPhotoID=$_REQUEST["txtPhotoID"];
		$qSQL="SELECT p_photo.AlbumID, p_album.AlbumName, p_album.AlbumNameBN, p_photo.PhotoID, p_photo.CaptionBN, p_photo.ImagePath, p_photo.ImageType, p_photo.ShowAtHome, p_photo.DateTimeInserted, p_photo.DateTimeUpdated, p_photo.ContentID FROM p_album INNER JOIN p_photo ON p_photo.AlbumID=p_album.AlbumID WHERE p_photo.Deletable=1 AND p_photo.ShowAtHome=1 AND p_photo.PhotoID=".$iPhotoID." ORDER BY p_photo.PhotoID DESC LIMIT $offset, $rowsPerPage";
	}
	if(isset($_REQUEST["btnSearch"])){
		$sSearchText=$_REQUEST["txtSearch"];
		$qSQL="SELECT p_photo.AlbumID, p_album.AlbumName, p_album.AlbumNameBN, p_photo.PhotoID, p_photo.CaptionBN, p_photo.ImagePath, p_photo.ImageType, p_photo.ShowAtHome, p_photo.DateTimeInserted, p_photo.DateTimeUpdated, p_photo.ContentID FROM p_album INNER JOIN p_photo ON p_photo.AlbumID=p_album.AlbumID WHERE p_photo.Deletable=1 AND p_photo.ShowAtHome=1 AND p_photo.Caption LIKE '%".$sSearchText."%' OR p_photo.CaptionBN LIKE '%".$sSearchText."%') ORDER BY p_photo.PhotoID DESC LIMIT $offset, $rowsPerPage";
	}
	//echo $qSQL."<br>";
	...
	<tr class="TrUpdateListSelect">
		<td align="left" valign="top">
			...
			<b>Caption:</b> <span class="spnBn"><?php echo $rsGallery["CaptionBN"]; ?></span>
		</td>
		<td align="left" valign="top">
			<?php $sImageFile=$sSiteURL."media/PhotoGallery/".$sImagePath;
			$iSize=filesize($sImageFile)/1024;
			if($iSize>150){$iSize="<font color='red' size='4'>".$iSize."<font>";}else{$iSize=$iSize;} ?>
	...
	if($iAlbumID<=0){
		$qSQL="SELECT COUNT(PhotoID) AS numrows FROM p_photo WHERE Deletable=1 AND p_photo.ShowAtHome=1 ";
	}else{
		$qSQL="SELECT COUNT(PhotoID) AS numrows FROM p_photo WHERE Deletable=1 AND p_photo.ShowAtHome=1 AND AlbumID=".$iAlbumID;
	}
