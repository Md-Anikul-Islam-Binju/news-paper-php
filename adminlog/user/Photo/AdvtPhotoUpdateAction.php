...
if(isset($_POST["btnSubmit"])){
	...
	if($sImageSMPath!=""){
		$qUpdate="UPDATE p_advt SET Position=".$iPosition.", Caption='".$sCaption."', URL='".$sURL."', ImagePath='".$sImageSMPathNew."' WHERE AdvtImageID=".$iUpdateID;
		$resultUpdate=mysqli_query($connEMM, $qUpdate) or die(mysqli_error($connEMM));
	}else{
		$qUpdate="UPDATE p_advt SET Position=".$iPosition.", Caption='".$sCaption."', URL='".$sURL."' WHERE AdvtImageID=".$iUpdateID;
		$resultUpdate=mysqli_query($connEMM, $qUpdate) or die(mysqli_error($connEMM));
	}
	//echo $qUpdate."<br><br>";
	...
	if(isset($resultUpdate)){
		...
		VALUES('".$_SESSION["sessUserName"]."', 2, ".$iUpdateID.", 'p_advt', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qUpdate)."', '".$dtDateTime."')";
					...
					while($fileRen=readdir($dir)){
						if($fileRen==$sImageSMPath){
							...
							if($iFlag==1){
								...
								$qUpdate="UPDATE p_advt SET ImagePath='".$sNewName."' WHERE AdvtImageID=".$iUpdateID;
