...
$qDelete="UPDATE tv_webtv SET Deletable=2 WHERE WebTVID=".$iDeleteID;
//echo $qDelete."<br>";
...
if($resultDelete){
	...
	VALUES('".$_SESSION["sessUserName"]."', 3, ".$iDeleteID.", 'tv_webtv', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qDelete)."', '".$dtDateTime."')";
	mysqli_query($connEMMAudit, $qAuditTrail) or die($sMsgAuTrailInsert);
	echo $sMsgDelete;
	//header("Location: webTVInsert.php");
	header("Location: generateWebTV.php");
}else{
	echo $sMsgDeleteFail;
} ?>
